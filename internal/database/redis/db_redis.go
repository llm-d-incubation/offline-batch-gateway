...
import (
	...
	"github.ibm.com/WML/watsonx-inference-proxy/internal/redis"
)
...
type BatchDBClientRedis struct {
	...
	redisClientChecker *redis.RedisClientChecker
	timeout            time.Duration
	idleLogFreq        time.Duration
	idleLogLast        time.Time
}

func NewBatchDBClientRedis(conf *config.BatchConfig, serviceName string, logger logging.Log) (
	*BatchDBClientRedis, error) {

	if logger == nil {
		logger = logging.GetInstance()
	}
	if conf.RedisClientConf == nil {
		err := fmt.Errorf("empty redis config")
		logger.Error(err, "NewBatchDBClientRedis:")
		return nil, err
	}
	redisClient, err := redis.NewRedisClient(conf.RedisClientConf, serviceName+redisClientNameSuf, logger)
	if err != nil {
		logger.Error(err, "NewBatchDBClientRedis: NewRedisClient failed")
		return nil, err
	}
	redisClientChecker := redis.NewRedisClientChecker(redisClient, keysPrefix, serviceName, conf.RedisClientConf.Timeout)
	logger.Info("NewBatchDBClientRedis: succeeded", "serviceName", serviceName)
	return &BatchDBClientRedis{
		redisClient:        redisClient,
		redisClientChecker: redisClientChecker,
		timeout:            conf.RedisClientConf.Timeout,
		idleLogFreq:        logFreqDefault,
		idleLogLast:        time.Now(),
	}, nil
}

func (c *BatchDBClientRedis) Close() (err error) {
...
func (c *BatchDBClientRedis) Store(ctx context.Context, logger logging.Log, job *db_api.BatchJob) (
	ID string, err error) {
...
func (c *BatchDBClientRedis) GetForProcessing(
	ctx context.Context, logger logging.Log, timeout time.Duration, maxJobs int) (
	...
	// Get job IDs from the priority queue.
	cctx, ccancel := context.WithTimeout(ctx, timeout+2*time.Second)
	_, vals, err := c.redisClient.BZMPop(
		cctx, timeout, goredis.Min.String(), int64(maxJobs), priorityQueueKeyName).Result()
	ccancel()
	if err != nil {
		if unrecognizedBlockingError(err) {
			logger.Error(err, "GetForProcessing: BZMPop failed")
			cerr := c.redisClientChecker.Check(ctx, logger)
			if cerr != nil {
				logger.Error(err, "GetForProcessing: ClientCheck failed")
			}
			return nil, nil, err
		}
		if time.Since(c.idleLogLast) >= c.idleLogFreq {
			logger.Info("GetForProcessing: no jobs")
...
func (c *BatchDBClientRedis) Update(ctx context.Context, logger logging.Log, job *db_api.BatchJob, addJobToQueue bool) (err error) {
	...
	logger = logger.WithValues("batchId", job.ID)
	if len(job.Status) == 0 && len(job.Tags) == 0 && !addJobToQueue {
		logger.Info("Update: nothing to update")
		return
	}
	if addJobToQueue && job.SLO.IsZero() {
		err = fmt.Errorf("valid SLO is required for adding a job to the queue")
		logger.Error(err, "Update:")
		return
	}
	...
	cmds, err := c.redisClient.Pipelined(cctx, func(pipe goredis.Pipeliner) error {
		...
		if addJobToQueue {
			item := goredis.Z{
				Score:  float64(job.SLO.UnixMicro()),
				Member: job.ID,
			}
			pipe.ZAddNX(cctx, priorityQueueKeyName, item)
		}
		return nil
	})
	ccancel()
	...
	logger.Info("Update: succeeded", "updatedStatus", updatedStatus, "updatedTags", updatedTags, "addJobToQueue", addJobToQueue)

	return
}

func (c *BatchDBClientRedis) Delete(ctx context.Context, logger logging.Log, IDs []string) (
	...
	cmds, err := c.redisClient.Pipelined(cctx, func(pipe goredis.Pipeliner) error {
		for _, id := range IDs {
			pipe.ZRem(cctx, priorityQueueKeyName, id)
...
func (c *BatchDBClientRedis) SendEvent(ctx context.Context, logger logging.Log, events []db_api.BatchEvent) (
...
func (c *BatchDBClientRedis) Get(
	ctx context.Context, logger logging.Log, IDs []string, tags []string,
...
func (c *BatchDBClientRedis) GetContext(parentCtx context.Context, timeLimit time.Duration) (context.Context, context.CancelFunc) {
	if parentCtx == nil {
...
func dbJobFromHget(vals []interface{}, includeStatic bool, logger logging.Log) (*db_api.BatchJob, error) {
